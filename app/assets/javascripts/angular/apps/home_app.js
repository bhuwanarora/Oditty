var homeApp = angular.module('homeApp', ['ngAnimate', 'ngMaterial', 'ngMessages', 'duScroll', 'ngRoute', 'monospaced.mousewheel', 'appConstants', 'timer', 'duScroll', 'filtersApp', 'angular.filter', 'angular-parallax', 'ui.router', 'ngCookies', 'ngSanitize', 'facebook']);
...
homeApp.run(["$rootScope", "$location", "$cookieStore", "$cookies", "$http", "$state", function($rootScope, $location, $cookieStore, $cookies, $http, $state){
    ...
    if(unauthenticated_user){
        var closed_urls = ($location.$$absUrl.indexOf("signup") < 0) && ($location.$$absUrl.indexOf("book") < 0) && ($location.$$absUrl.indexOf("author") < 0) && ($location.$$absUrl.indexOf("communities") < 0) && ($location.$$absUrl.indexOf("home") < 0) && ($location.$$absUrl.indexOf("room") < 0) && ($location.$$absUrl.indexOf("news") < 0) && ($location.$$absUrl.indexOf("news_group") < 0) && ($location.$$absUrl.indexOf("filters") < 0) && ($location.$$absUrl.indexOf("publisher") < 0) && ($location.$$absUrl.indexOf("search") < 0) && ($location.$$absUrl.indexOf("games") < 0) && ($location.$$absUrl.indexOf("profile") < 0) && ($location.$$absUrl.indexOf("quiz") < 0) && ($location.$$absUrl.indexOf("spaces") < 0) && ($location.$$absUrl.indexOf("testimonials") < 0) && ($location.$$absUrl.indexOf("journey") < 0) && ($location.$$absUrl.split("/")[3] != "") && ($location.$$absUrl.indexOf("products") < 0) ;
...
function setCookie(cname, cvalue, exdays) {
    ...
    document.cookie = cname + "=" + cvalue + "; " + expires + "; path=/;domain=oditty.me";
}
