	...
	def create_for reading_status_value 
		...
		unless reading_status_value.nil?
			...
			when Constant::StatusCode::PlanningToRead
				clause = Status::StatusType::PlanningToRead.new(@book_id, @user_id).create + Book::Feed.new(@book_id).create("status") 
			when Constant::StatusCode::CurrentlyReading
				clause = Status::StatusType::CurrentlyReading.new(@book_id, @user_id).create + Book::Feed.new(@book_id).create("status")
			when Constant::StatusCode::Read
				clause = Status::StatusType::Read.new(@book_id, @user_id).create + Book::Feed.new(@book_id).create("status")
			end
		end 
		clause
	end
