class Status::Mention::MentionsUser < Status::Mention
	def initialize mentioned_user_id, user_id
		...
		@notification_node_variable = "status"
	end
	...
	def self.create_group mentioned_users_id, user_id
		User.match_group(mentioned_users_id) + Status::Mention::MentionsUser.create_for_user(user_id) + User::UserNotification.add(@notification_node_variable)   
	end
