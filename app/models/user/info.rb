class User::Info < User
	...
	def self.set_last_login 
		 " SET user.last_login = \"" + Time.now.strftime("%Y-%m-%d") + "\", user.login_count = COALESCE(user.login_count,0) + 1 "
	end
	...
	def self.set_followed_by_count operation
		" SET user.followed_by_count = COALESCE(user.followed_by_count,0)" + operation + "1 "
	end
