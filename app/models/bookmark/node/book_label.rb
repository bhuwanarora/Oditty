class Bookmark::Node::BookLabel < Bookmark::Node
	...
	def self.get_public user_id
		Label.match_public + ", user "  + Label.optional_match_books + Bookmark::Node::BookLabel.return_group("DISTINCT label.name AS shelf", "label.key as label_key", "book AS books", "label_count AS label_count")
	end
