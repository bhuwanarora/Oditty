class Search < Neo
	...
	def book_by_title
		Search.match_indexed(Constant::IndexName::BookTitle, ( @search_text + @connector)) + Search.return_group(Book.basic_info,"labels(node) as labels").search_compliant + Search.order_by("popularity") + " DESC " + Search.skip(@skip_count) + Search.limit(@count) 
	end
