class UsersNews < Neo
	...
	# 	rating_clause = _match_user_and_book(@user_id, @news)+" CREATE UNIQUE (u)-[:TimingAction]->(m:TimingNode{news:"+@news.to_s+", title:b.title, author:b.author_name, user_id:"+@user_id.to_s+"})-[:Timer]->(b) SET m.timestamp="+Time.now.to_i.to_s+", m.time_index="+time.to_s+", m.name=u.name, m.email=u.email, m.isbn=b.isbn, m.thumb = CASE WHEN u.thumb IS NULL THEN '' ELSE u.thumb END  WITH u, b, m "
	...
	# 	clause = _recommend_clause(@user_id, @news, friend_id) + _feednext_clause(@user_id) + _bookfeed_clause(@user_id) + _existing_ego_clause + _ego_clause + total_count + recommended_count
	# 	clause
	# end

	# def self.create_thumb_request(thumb_url)
	# 	thumb_request_clause = "MATCH (u:User), (b:Book) WHERE ID(u)="+@user_id.to_s+" AND ID(b) = "+@news.to_s+" CREATE UNIQUE (u)-[:DataEdit]->(t:ThumbRequest{url: \""+thumb_url+"\", user_id: "+@user_id.to_s+", news: "+@news.to_s+"})-[:DataEditRequest]->(b) SET t.name=u.name, t.email=u.email, t.thumb=u.thumb, t.title=b.title, t.author_name=b.author_name, t.isbn=b.isbn, t.timestamp = "+Time.now.to_i.to_s+" WITH u, b, t "
	...
	# 	#Add category 
	# 	"MATCH (b:Book{id:"+@news+"})-[h1:Has]->(t:Tag)<-[h2:Has]-(sb:Book), 
	# 		(u:User{id:"+@user_id+"})
	# 		WHERE b <> sb AND
