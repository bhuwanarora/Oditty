	...
	module V0
		class WebsiteApi
			...
			def self.get_root_categories user_id=nil
				neo_init
				clause = "MATCH (category:Category) WHERE category.is_root = true WITH category OPTIONAL MATCH (category)<-[r:Likes]-(u:User) WHERE ID(u)="+user_id.to_s+" AND ALL(relation IN R WHERE r.weight > 0) RETURN category.name as name, ID(category) as id, category.icon as icon, ID(r) as likes, category.aws_key as key LIMIT 28"
