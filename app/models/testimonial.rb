class Testimonial < Neo
	...
	def self.get_all skip=0
		Testimonial.match + Testimonial.return_group("user.name AS name", "user.thumb AS image_url", Testimonial.basic_info) + Testimonial.order_by("testimonial.created_at") + Testimonial.skip(skip) + Testimonial.limit(10)
	end
