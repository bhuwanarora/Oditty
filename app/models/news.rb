class News < Neo
	...
	def self.basic_info
		" ID(news) AS  id  ,news.url  AS url , news.image_url AS image_url, news.title AS title, news.description AS description, news.created_at AS created_at, labels(news) AS label, news.search_index AS search_index "
	end
