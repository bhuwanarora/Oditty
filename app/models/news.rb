...
#{relevance: "+ relevance['relevance'].to_s+",relevanceOriginal:"+relevance['relevanceOriginal'].to_s+"}
	...
	def self.get_feed skip_count, day_skip_count, region
		News.match_time_period(day_skip_count) + " WHERE news.status = true WITH news " + News.match_region(region) + News.order_desc + News.skip(skip_count) + News.limit(Constant::Count::NewsShownInFeed) + News.match_community + UsersCommunity.optional_match + ", news " + News.limit(Constant::Count::UsersShownInFeed) + " WITH DISTINCT news, user WITH DISTINCT news , " + News.collect_map({"users" => User.grouped_primary_display_info}) + News.match_community + ", SUM(community.follow_count) AS follow_count, users ORDER BY has_community.relevance DESC WITH DISTINCT users, follow_count, news,  " + News.collect_map({"communities" => Community.grouped_basic_info}) + " WITH DISTINCT news, users, communities, follow_count " + News.return_group(News.basic_info,"follow_count, communities[0.." +  Constant::Count::CommunitiesOfNewsShown.to_s + "] AS communities ", "users") 
	end
