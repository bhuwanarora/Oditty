module UsersGraphHelper
	...
	def self.create_user(email, password=nil, verification_token=nil)
		@neo ||= self.neo_init
		clause = "MATCH (all_user:User) CREATE (user:User{email:\""+email+"\", verification_token:\""+verification_token+"\", password:\""+password+"\", like_count:0, indexed_email: \""+email+"\", rating_count:0, timer_count:0, dislike_count:0, comment_count:0, bookmark_count:0, book_read_count:0, follows_count:0, followed_by_count:0}), (user)-[fn:FeedNext{user_id:ID(user)}]->(user), (user)-[:Ego{user_id:ID(user)}]->(user), (user)-[:Follow]->(all_user), (user)<-[:Follow]-(all_user) WITH user MATCH(bm:Label{primary_label:true}) CREATE (user)-[:Labelled{user_id:ID(user)}]->(bm)"
		puts clause.blue.on_red
		@neo.execute_query(clause)
	end
