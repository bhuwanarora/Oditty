module PersonalisedBookSuggestionHelper
	...
	def self.get_small_reads
		get_top_tiny_reads = " MATCH path = (book)-[:NextSmallRead*" + Constant::Count::BookRecommendation.to_s + "]->(tiny_read) WHERE ID(book) = " + Constant::Id::BestSmallRead.to_s + " WITH EXTRACT (n IN nodes(path)|n) AS books UNWIND books AS book RETURN "
	...
	def self.get_books_from_favourite_author user_id
		...
		unread_books_from_author_clause = " WITH author, book ORDER BY book.total_weight DESC LIMIT " + Constant::Count::BookRecommendation.to_s
	...
	def self.get_books_from_most_bookmarked_era user_id
		...
		unread_books_from_era_clause = " WITH era, book ORDER BY book.total_weight DESC LIMIT " + Constant::Count::BookRecommendation.to_s 
	...
	def self.get_books_on_friends_shelves user_id
		friends_reads_clause = " OPTIONAL MATCH (user)-[:Follows]->(friend:User)-->(:Label)-->(:BookmarkNode)-->(book:Book) WHERE NOT (user)-->(:Label)-->(:BookmarkNode)-->(book) WITH friend, book ORDER BY book.total_weight DESC LIMIT " + Constant::Count::BookRecommendation.to_s
