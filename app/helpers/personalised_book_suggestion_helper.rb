module PersonalisedBookSuggestionHelper
	...
	def self.get_tiny_reads
		get_top_tiny_reads = " MATCH path = (book)-[:NextTinyRead*" + Constants::RecommendationBookCount.to_s + "]->(tiny_read) WHERE ID(book) = " + Constants::BestTinyRead.to_s + " WITH EXTRACT (n IN nodes(path)|n) AS books UNWIND books AS book RETURN "
	...
	def self.get_books_from_most_bookmarked_author user_id
