module PersonalisedBookSuggestionHelper
	...
	def self.get_books_from_favourite_category(user_id, favourites = true)
		...
		while data.length < 10 
			...
			match_category_to_books_clause  = " MATCH path = (book)-[:NextInCategory*" + (Constants::RecommendationBookCount*10).to_s + "]-(next_book) WHERE ALL(relation IN relationships(path) WHERE relation.uuid = root_category.uuid) WITH mean_deviation, user, root_category, FILTER(node IN nodes(path) WHERE NOT (user)-[:Labelled]->(:Label)-[:BookmarkedOn]->(:BookmarkNode)-[:BookmarkAction]->(node)) AS books UNWIND books AS book WITH book, mean_deviation, root_category ORDER BY book.total_weight LIMIT " + Constants::RecommendationBookCount.to_s 
	...
	def self.get_books_from_most_bookmarked_era user_id
		...
		unread_books_from_era_clause = " WITH era, book ORDER BY book.total_weight DESC LIMIT " + Constants::RecommendationBookCount.to_s 
	...
	def self.get_books_on_friends_shelves user_id
		friends_reads_clause = " OPTIONAL MATCH (user)-[:Follows]->(friend:User)-->(:Label)-->(:BookmarkNode)-->(book:Book) WHERE NOT (user)-->(:Label)-->(:BookmarkNode)-->(book) WITH friend, book ORDER BY book.total_weight DESC LIMIT " + Constants::RecommendationBookCount.to_s
