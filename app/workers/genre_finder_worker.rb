module GenreFinderWorker
	...
	def self.find_genres_for_book user_id. book_title
		...
		relate_user_categories_clause = " MATCH (book)-[from_category:FromCategory]->(category:Category)-[:HasRoot*0..1]->(root_category{is_root:true}) WITH root_category MERGE (user)-[rel:Likes]-(root_category) WHERE ID(user) = " + user_id.to_s + " ON MATCH SET rel.weight = rel.weight + 1" 
